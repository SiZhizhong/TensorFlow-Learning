...
class MNISTModel(TFModel):
    ...
    def run_epoch(self, sess, input_data=None, input_labels=None):
        with tf.name_scope("summaries"):
            ...
            tf.summary.histogram("loss_histogram", self.loss_op)
        ...
        for _ in range(n_batch):
            ...
            _, loss, summary = sess.run([self.optimizer, self.loss_op, self.all_summary], feed_dict=feed_dict)
            total_loss += loss
        average_loss = total_loss / n_batch
        return (average_loss, summary)
    ...
    def fit(self, sess, input_data=None, input_labels=None):
        ...
        for epoch in range(self.config.epochs):
            epoch_loss, summary = self.run_epoch(sess, input_data, input_labels)
